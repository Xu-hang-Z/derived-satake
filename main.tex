...
\begin{equation}\label{non-equiv formula}
    h\cdot v_i=iv_i,\quad e\cdot v_i=\frac{n+i}{2} v_{i+1}, \quad f\cdot v_i=\frac{n-i}{2} v_{i-1}.
\end{equation}
...
$$ \iota_i^!(\alpha)=[\ft_i\cap\Gr_n]\cap\iota_i^*(\alpha)= \iota_i^*\iota_{i*}[\ft_i\cap \Gr_n]\cap\iota_i^*(\alpha)=e(N_{\ft_k\cap\Gr_n}(\Gr_n))\cdot \iota_i^*(\alpha).$$
...
\begin{equation}
    \tilde{v}_i= (h+(i-1)\hbar)(h+(i-2)\hbar)\dots\ (h+\frac{i-n}{2}\hbar)\tilde{v}_{-n}.
\end{equation}
